...
trait Relations {
  ...
  /** All files that are recorded as an auxiliary dependency of a source file in this group. */
  def allAuxiliaryDeps: collection.Set[File]
  ...
  /** Auxiliary dependencies that internal source file `src` depends on. */
  def auxiliaryDeps(src: File): Set[File]
  /** The source files that depend on auxiliary file `dep`. */
  def usesAuxiliary(dep: File): Set[File]
  ...
  def addSource(src: File,
    ...
    auxiliaryDeps: Iterable[(File, Stamp)]): Relations =
    addProducts(src, products).addInternalSrcDeps(src, internalDeps).addExternalDeps(src, externalDeps).addBinaryDeps(src, binaryDeps).addAuxiliaryDeps(src, auxiliaryDeps)
  ...
  /**
   * Records all the auxiliary dependencies `deps` of `src`
   */
  private[inc] def addAuxiliaryDeps(src: File, deps: Iterable[(File, Stamp)]): Relations
  ...
  /** The dependency relation between internal sources and auxiliary files. */
  def auxiliaryDep: Relation[File, File]
...
object Relations {
  ...
  private[inc] val existingRelations = {
    ...
    List(
      ...
      ("auxiliary dependencies", string2File),
    ...
    relations match {
      case p :: bin :: aux :: di :: de :: pii :: pie :: mri :: mre :: ii :: ie :: mei :: mee :: mi :: me :: cn :: un :: Nil =>
        ...
        if (nameHashing) {
          ...
          Relations.make(srcProd, binaryDep, auxiliaryDep, directSrcDeps, publicInheritedSrcDeps, classes)
        }
  ...
  private[inc] def empty(nameHashing: Boolean): Relations =
    if (nameHashing)
      new MRelationsNameHashing(e, e, e, InternalDependencies.empty, ExternalDependencies.empty, estr, estr)
    else
      new MRelationsDefaultImpl(e, e, e, es, es, estr)

  def make(srcProd: Relation[File, File], binaryDep: Relation[File, File], auxiliaryDep: Relation[File, File], direct: Source, publicInherited: Source, classes: Relation[File, String]): Relations =
    new MRelationsDefaultImpl(srcProd, binaryDep, auxiliaryDep, direct = direct, publicInherited = publicInherited, classes)

  private[inc] def make(srcProd: Relation[File, File], binaryDep: Relation[File, File], auxiliaryDep: Relation[File, File],
    ...
    new MRelationsNameHashing(srcProd, binaryDep, auxiliaryDep, internalDependencies = internalDependencies, externalDependencies = externalDependencies, classes, names)
...
private abstract class MRelationsCommon(val srcProd: Relation[File, File], val binaryDep: Relation[File, File],
    val auxiliaryDep: Relation[File, File], val classes: Relation[File, String]) extends Relations {
  ...
  def allAuxiliaryDeps: collection.Set[File] = auxiliaryDep._2s
  ...
  def auxiliaryDeps(src: File): Set[File] = auxiliaryDep.forward(src)
  def usesAuxiliary(dep: File): Set[File] = auxiliaryDep.reverse(dep)
...
private class MRelationsDefaultImpl(srcProd: Relation[File, File], binaryDep: Relation[File, File],
    auxiliaryDep: Relation[File, File],
    // direct should include everything in inherited
    val direct: Source, val publicInherited: Source,
    classes: Relation[File, String]) extends MRelationsCommon(srcProd, binaryDep, auxiliaryDep, classes) {
  ...
  def addProducts(src: File, products: Iterable[(File, String)]): Relations =
    new MRelationsDefaultImpl(srcProd ++ products.map(p => (src, p._1)), binaryDep, auxiliaryDep, direct = direct,
  ...
  def addInternalSrcDeps(src: File, deps: Iterable[InternalDependency]) = {
    ...
    new MRelationsDefaultImpl(srcProd, binaryDep, auxiliaryDep, direct = newD, publicInherited = newI, classes)
  }
  ...
  def addBinaryDeps(src: File, deps: Iterable[(File, String, Stamp)]) =
    new MRelationsDefaultImpl(srcProd, binaryDep + (src, deps.map(_._1)), auxiliaryDep, direct, publicInherited, classes)

  def addBinaryDep(src: File, dependsOn: File): Relations =
    new MRelationsDefaultImpl(srcProd, binaryDep + (src, dependsOn), auxiliaryDep, direct = direct,
      publicInherited = publicInherited, classes)

  def addAuxiliaryDeps(src: File, deps: Iterable[(File, Stamp)]) =
    new MRelationsDefaultImpl(srcProd, binaryDep, auxiliaryDep + (src, deps.map(_._1)), direct, publicInherited, classes)
  ...
  def ++(o: Relations): Relations = {
    ...
    new MRelationsDefaultImpl(srcProd ++ o.srcProd, binaryDep ++ o.binaryDep, auxiliaryDep ++ o.auxiliaryDep,
      direct ++ o.direct, publicInherited ++ o.publicInherited, classes ++ o.classes)
  }
  def --(sources: Iterable[File]) =
    new MRelationsDefaultImpl(srcProd -- sources, binaryDep -- sources, auxiliaryDep = auxiliaryDep -- sources, direct = direct -- sources, publicInherited = publicInherited -- sources, classes -- sources)

  @deprecated("Broken implementation. OK to remove in 0.14", "0.13.1")
  def groupBy[K](f: File => K): Map[K, Relations] =
    {
      type MapRel[T] = Map[K, Relation[File, T]]
      def outerJoin(srcProdMap: MapRel[File], binaryDepMap: MapRel[File], auxDepMap: MapRel[File], direct: Map[K, Source],
        ...
        namesMap: MapRel[String]): Map[K, Relations] =
        {
          def kRelations(k: K): Relations = {
            ...
            new MRelationsDefaultImpl(get(srcProdMap), get(binaryDepMap), get(auxDepMap), getSrc(direct), getSrc(inherited),
              get(classesMap))
          }
      ...
      outerJoin(srcProd.groupBy(f1), binaryDep.groupBy(f1), auxiliaryDep.groupBy(f1), direct.groupBySource(f),
        publicInherited.groupBySource(f), classes.groupBy(f1), names.groupBy(f1))
    }

  override def equals(other: Any) = other match {
    case o: MRelationsDefaultImpl =>
      srcProd == o.srcProd && binaryDep == o.binaryDep && auxiliaryDep == o.auxiliaryDep &&
        direct == o.direct && publicInherited == o.publicInherited && classes == o.classes
    case _ => false
  }
  ...
  override def toString = (
    """
	  ...
	  |  ext deps: %s
	  |  class names: %s
	  """.trim.stripMargin.format(List(srcProd, binaryDep, auxiliaryDep, internalSrcDep, externalDep, classes) map relation_s: _*)
  )
}
...
private class MRelationsNameHashing(srcProd: Relation[File, File], binaryDep: Relation[File, File],
    ...
    val names: Relation[File, String]) extends MRelationsCommon(srcProd, binaryDep, auxiliaryDep, classes) {
  def direct: Source =
  ...
  def addProduct(src: File, prod: File, name: String): Relations =
    new MRelationsNameHashing(srcProd + (src, prod), binaryDep, auxiliaryDep, internalDependencies = internalDependencies,
      externalDependencies = externalDependencies, classes + (src, name), names = names)

  def addProducts(src: File, products: Iterable[(File, String)]): Relations =
    new MRelationsNameHashing(srcProd ++ products.map(p => (src, p._1)), binaryDep, auxiliaryDep,
      internalDependencies = internalDependencies, externalDependencies = externalDependencies,
      classes ++ products.map(p => (src, p._2)), names = names)

  def addInternalSrcDeps(src: File, deps: Iterable[InternalDependency]) =
    new MRelationsNameHashing(srcProd, binaryDep, auxiliaryDep, internalDependencies = internalDependencies ++ deps,
  ...
  def addExternalDeps(src: File, deps: Iterable[ExternalDependency]) =
    new MRelationsNameHashing(srcProd, binaryDep, auxiliaryDep, internalDependencies = internalDependencies,
  ...
  def addBinaryDeps(src: File, deps: Iterable[(File, String, Stamp)]) =
    new MRelationsNameHashing(srcProd, binaryDep + (src, deps.map(_._1)), auxiliaryDep, internalDependencies = internalDependencies,
      externalDependencies = externalDependencies, classes, names)

  def addBinaryDep(src: File, dependsOn: File): Relations =
    new MRelationsNameHashing(srcProd, binaryDep + (src, dependsOn), auxiliaryDep, internalDependencies = internalDependencies,
      externalDependencies = externalDependencies, classes, names = names)

  def addAuxiliaryDeps(src: File, deps: Iterable[(File, Stamp)]) =
    new MRelationsNameHashing(srcProd, binaryDep, auxiliaryDep + (src, deps.map(_._1)), internalDependencies = internalDependencies,
      externalDependencies = externalDependencies, classes, names = names)
  ...
  def ++(o: Relations): Relations = {
    if (!o.nameHashing)
      throw new UnsupportedOperationException("The `++` operation is not supported for relations " +
        "with different values of `nameHashing` flag.")
    new MRelationsNameHashing(srcProd ++ o.srcProd, binaryDep ++ o.binaryDep, auxiliaryDep ++ o.auxiliaryDep,
  ...
  def --(sources: Iterable[File]) =
    new MRelationsNameHashing(srcProd -- sources, binaryDep -- sources, auxiliaryDep -- sources,
  ...
  override def equals(other: Any) = other match {
    case o: MRelationsNameHashing =>
      srcProd == o.srcProd && binaryDep == o.binaryDep && auxiliaryDep == o.auxiliaryDep &&
        memberRef == o.memberRef && inheritance == o.inheritance && classes == o.classes
    case _ => false
  }
  ...
  override def toString = (
    """
	  ...
	  |  class names: %s
	  |  used names: %s
	  """.trim.stripMargin.format(List(srcProd, binaryDep, auxiliaryDep, internalSrcDep, externalDep, classes, names) map relation_s: _*)
  )
